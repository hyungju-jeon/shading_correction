...
def BaSiC_parameters():
    return {
        ...
        "eplson": 0.1,  # reweighting parameter
        "varying_coeff": True,
...
def inexact_alm_rspca_l1(
    ...
    *,
    lambdaa=None,
    lambda_darkfield=None,
    tol=1e-6,
    maxIter=500,
    weight=1.0,
    estimatedarkfield=False,
    darkfieldlimit=1e7,
):
    """
    ...
    A_inmask[
        int(np.round(p / 6.0)) : int(np.round(p * 5.0 / 6)),
        int(np.round(q / 6.0)) : int(np.round(q * 5.0 / 6)),
    ] = 1
    # main iterations
    iter = 0
    total_svd = 0
    ...
    while not converged:
        ...
        W_hat = np.fmax(W_hat - lambdaa / (ent1 * mu), 0.0) + np.fmin(
            W_hat + lambdaa / (ent1 * mu), 0.0
        )
        W_idct_hat = scipy.fft.idctn(W_hat, norm="ortho")
        A1_hat = W_idct_hat.reshape((-1, 1)) @ A1_coeff.reshape((1, -1)) + A_offset.reshape((-1, 1))
        ...
        if estimatedarkfield:
            validA1coeff_idx = A1_coeff < 1.0
            W_idct_hat_flatten = W_idct_hat.flatten()
            B1_coeff = (
                np.mean(
                    R1[W_idct_hat_flatten > np.mean(W_idct_hat_flatten) - 1e-6][
                        :, validA1coeff_idx
                    ],
                    axis=0,
                )
            ...
            A1_offset = (
                np.mean(R1[:, validA1coeff_idx], axis=1)
                - np.mean(A1_coeff[validA1coeff_idx]) * W_idct_hat_flatten
            )
            ...
            W_offset = np.fmax(W_offset - lambda_darkfield / (ent2 * mu), 0) + np.fmin(
            ...
            A_offset = np.fmax(A_offset - lambda_darkfield / (ent2 * mu), 0) + np.fmin(
...
def BaSiC(
    ...
    *,
    ...
    if lambdaa is None:
        lambdaa = np.abs(W_meanD).sum() / 400.0 * 0.5
    if lambda_darkfield is None:
        lambda_darkfield = np.abs(W_meanD).sum() / 400.0 * 0.2
    ...
    while flag_reweighting:
        ...
        X_k_A, X_k_E, X_k_Aoffset = inexact_alm_rspca_l1(
            D,
            lambdaa=lambdaa,
            ...
            weight=weight,
            estimatedarkfield=estimate_darkfield,
        )
        ...
        mad_flatfield = (
            np.abs(flatfield_current - flatfield_last).sum() / np.abs(flatfield_last).sum()
        )
