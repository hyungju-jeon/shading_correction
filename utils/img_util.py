...
def pad_img(
    ...
    *,
    ...
    do_padding = (
        (des_channel > 0 and des_channel != nchs)
        or (des_depth > 0 and des_depth != depth)
...
def write_img(
    ...
    *,
    ...
    pad_data = np.ascontiguousarray(
        pad_img(
            img_data,
            des_channel=des_channel,
            des_depth=des_depth,
            des_height=des_height,
            des_width=des_width,
            pad_before_ratio=pad_before_ratio,
        )
    )
...
def read_img_as_RGB(filename: str):
    ...
    for ch in range(nchs):
        ...
        res[0, :, :, :] = np.maximum(
            res[0, :, :, :], (img.info.channelColors[ch].r * image).astype(np.uint8)
        )
        res[1, :, :, :] = np.maximum(
            res[1, :, :, :], (img.info.channelColors[ch].g * image).astype(np.uint8)
        )
        res[2, :, :, :] = np.maximum(
            res[2, :, :, :], (img.info.channelColors[ch].b * image).astype(np.uint8)
        )
    return res
...
def normalize_img_data(img_data: np.ndarray, min_max_percentile=(2, 98)):
    ...
    for ch, chdata in enumerate(img_data):
        if min_max_percentile[0] <= 0 and min_max_percentile[1] >= 100:
            ...
            minmax_value = np.percentile(
                chdata[chdata > 0], [min_max_percentile[0], min_max_percentile[1]]
            )
    ...
    *,
    ...
    if do_resizing:
        if img_data.ndim == 1:
            ...
            return (
                img.data[0]
                .reshape(orig_shape[0:-3] + (img.info.depth, img.info.height, img.info.width))
                .copy()
            )
    else:
        return img_data
